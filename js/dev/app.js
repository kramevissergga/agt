...
document.addEventListener("DOMContentLoaded", function() {
  var heroSliderEls = document.querySelectorAll(".hero__slider");
  ...
  var whyUsSliderEls = document.querySelectorAll(".why-us__slider");
  whyUsSliderEls.forEach(function(sliderEl) {
    var whyUsSlider = new Splide(sliderEl, {
      perPage: 2,
      arrows: true,
      pagination: true,
      perMove: 1,
  ...
  var articlesSliderEls = document.querySelectorAll(".posts-section__slider");
  articlesSliderEls.forEach(function(sliderEl) {
    var articlesSlider = new Splide(sliderEl, {
      perPage: 2,
      arrows: true,
      pagination: true,
      perMove: 1,
  ...
  var advantagesSliderEls = document.querySelectorAll(
    ".advantages-sust__slider"
  );
  advantagesSliderEls.forEach(function(sliderEl) {
    var advantagesSlider = new Splide(sliderEl, {
      perPage: 2,
      arrows: false,
      pagination: false,
      perMove: 1,
...
const mapEl$4 = document.getElementById("map");
function initMap$4() {
  const coordinates = mapEl$4.getAttribute("data-coordinates").split(",");
  ...
  let map = new google.maps.Map(mapEl$4, mapProp);
...
const mapEl$3 = document.getElementById("map");
function initMap$3() {
  const coordinates = mapEl$3.getAttribute("data-coordinates").split(",");
  const lat = parseFloat(coordinates[0]);
  const lng = parseFloat(coordinates[1]);
  let mapProp = {
    center: new google.maps.LatLng(lat, lng),
  ...
  let map = new google.maps.Map(mapEl$3, mapProp);
  new google.maps.Marker({
    position: new google.maps.LatLng(lat, lng),
    map
  });
...
if (mapEl$3) {
  window.onload = initMap$3;
}
function adjustTitleHeights() {
  if (window.innerWidth > 499.98) {
    const titles = document.querySelectorAll(".info-services__title");
    let maxHeight = 0;
...
window.addEventListener("load", adjustTitleHeights);
...
class DynamicAdapt {
  ...
  init() {
    ...
    this.nodes.forEach((node) => {
      ...
      const selector = dataArray[0] ? dataArray[0].trim() : null;
      ...
      if (selector && selector.startsWith("{") && selector.endsWith("}")) {
        const parentTag = selector.slice(1, -1);
        object.destination = node.closest(parentTag);
        if (!object.destination) ;
      } else if (selector) {
        object.destination = destinationParent.querySelector(selector);
        if (!object.destination) ;
      } else ;
...
const mapEl$2 = document.getElementById("map");
function initMap$2() {
  const coordinates = mapEl$2.getAttribute("data-coordinates").split(",");
  const lat = parseFloat(coordinates[0]);
  const lng = parseFloat(coordinates[1]);
  let mapProp = {
    center: new google.maps.LatLng(lat, lng),
  ...
  let map = new google.maps.Map(mapEl$2, mapProp);
  new google.maps.Marker({
    position: new google.maps.LatLng(lat, lng),
    map
  });
...
if (mapEl$2) {
  window.onload = initMap$2;
}
const mapEl$1 = document.getElementById("map");
function initMap$1() {
  const coordinates = mapEl$1.getAttribute("data-coordinates").split(",");
  const lat = parseFloat(coordinates[0]);
  const lng = parseFloat(coordinates[1]);
  let mapProp = {
    center: new google.maps.LatLng(lat, lng),
  ...
  let map = new google.maps.Map(mapEl$1, mapProp);
  new google.maps.Marker({
    position: new google.maps.LatLng(lat, lng),
    map
  });
